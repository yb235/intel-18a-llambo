...
- Observed backtest usable points: 1260.
...
## Thin-Data vs Enriched Comparison
- Prior thin observed set (`sample_observations`) used n=1 usable point(s); enriched observed set (`enriched_monthly_panel`) used n=105 usable point(s).
- Hardened LLAMBO RMSE changed from 1.9000 (thin) to 11.3984 (enriched proxy panel).
- Hardened LLAMBO coverage95 changed from 1.0000 (thin) to 0.4762 (enriched proxy panel).
- Hardened LLAMBO calibration_error changed from 0.3444 (thin) to 0.3974 (enriched proxy panel).
- Confidence assessment: sample-size confidence improved (far more backtest windows), but predictive-confidence did not improve because enriched `yield` is mostly a constructed proxy rather than measured fab-yield telemetry.
- Skeptical note: this enriched panel is useful for robustness stress testing and pipeline validation, not for high-stakes yield forecasting decisions.
